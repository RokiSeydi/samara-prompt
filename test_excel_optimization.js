...
async function testExcelOptimization() {
  try {
    ...
    if (!fs.existsSync('./sample_transportation_roster.xlsx')) {
      console.error('❌ Sample Excel file not found. Please run create_sample_excel.py first.');
      return;
    }
    ...
    console.log(`📈 Results:`);
    console.log(`  - Cost savings: $${optimizationResult.metrics.cost_savings.toFixed(2)}`);
    console.log(`  - Efficiency improvement: ${optimizationResult.metrics.efficiency_improvement.toFixed(1)}%`);
    console.log(`  - Conflicts resolved: ${optimizationResult.metrics.conflicts_resolved}`);

...
import { graphConfig, loginRequest } from '../config/msalConfig';
...
export const useGraphData = () => {
  ...
  const [accountType, setAccountType] = useState<'personal' | 'business' | 'unknown'>('unknown');
  ...
  const tryBusinessStandardEndpoints = async () => {
    ...
    const businessEndpoints = [
      ...
      { 
        ...
        endpoint: 'https://graph.microsoft.com/v1.0/me/drive/root/children?$top=25',
      ...
      { 
        ...
        endpoint: 'https://graph.microsoft.com/v1.0/me/memberOf/microsoft.graph.group',
      ...
      { 
        ...
        endpoint: 'https://graph.microsoft.com/v1.0/me/insights/trending?$top=25',
        description: 'Trending Office files'
      },
      // Alternative file access methods
      { 
        name: 'Shared Files', 
        endpoint: 'https://graph.microsoft.com/v1.0/me/drive/sharedWithMe?$top=25',
    ...
    for (const endpoint of businessEndpoints) {
      try {
        ...
        if (response.ok && !data.error) {
          ...
          console.log(`✅ ${endpoint.name}: SUCCESS - Found ${itemCount} items`);
          ...
          if (endpoint.name === 'OneDrive Personal' || endpoint.name === 'OneDrive Recent') {
            if (data.value && data.value.length > 0) {
              console.log(`📁 Found ${data.value.length} files in ${endpoint.name}`);
          ...
          if (endpoint.name === 'All Drives' && data.value) {
            for (const drive of data.value.slice(0, 3)) { // Try first 3 drives
              try {
                console.log(`💾 Checking drive: ${drive.name || drive.id}`);
                const driveFilesResponse = await callMsGraph(`https://graph.microsoft.com/v1.0/drives/${drive.id}/root/children?$top=25`);
                const driveFilesData = await driveFilesResponse.json();
                
                if (driveFilesResponse.ok && driveFilesData.value && driveFilesData.value.length > 0) {
                  console.log(`✅ Found ${driveFilesData.value.length} files in drive ${drive.name}`);
                  results.fileData = { 
          ...
          if (endpoint.name === 'SharePoint Sites' && data.value && data.value.length > 0) {
            for (const site of data.value.slice(0, 2)) { // Try first 2 sites
              try {
                console.log(`🏢 Checking SharePoint site: ${site.displayName}`);
                const siteDocsResponse = await callMsGraph(`https://graph.microsoft.com/v1.0/sites/${site.id}/drive/root/children?$top=25`);
                const siteDocsData = await siteDocsResponse.json();
                
                if (siteDocsResponse.ok && siteDocsData.value && siteDocsData.value.length > 0) {
                  console.log(`✅ Found ${siteDocsData.value.length} files in SharePoint site ${site.displayName}`);
                  results.fileData = { 
                ...
                console.log(`❌ SharePoint site ${site.displayName} failed:`, error.message);
              }
            }
          }
          
          // Handle Office Insights
          if ((endpoint.name === 'Office Insights Used' || endpoint.name === 'Office Insights Trending') && data.value && data.value.length > 0) {
            console.log(`✅ Found ${data.value.length} recent Office files via ${endpoint.name}`);
            const insightFiles = data.value
              .filter(item => item.resourceVisualization && item.resourceVisualization.type)
              .map(item => ({
                id: item.id,
                name: item.resourceVisualization.title,
                lastModifiedDateTime: item.lastUsed?.lastAccessedDateTime || new Date().toISOString(),
          ...
          console.log(`❌ ${endpoint.name}: FAILED - ${data.error?.message || response.statusText}`);
          results.failed.push({
            ...endpoint,
  ...
  const diagnoseBusinessAccount = async () => {
    try {
      ...
      const userResponse = await callMsGraph('https://graph.microsoft.com/v1.0/me');
      ...
      try {
        const subscriptionResponse = await callMsGraph('https://graph.microsoft.com/v1.0/me/licenseDetails');
        const subscriptionData = await subscriptionResponse.json();
        
        if (subscriptionResponse.ok && subscriptionData.value) {
          console.log('📋 License information:', subscriptionData.value.map(license => license.skuPartNumber));
        }
      } catch (error) {
        console.log('⚠️ Could not retrieve license information:', error.message);
      }
      
      // Check what Microsoft 365 services are available
      console.log('🔍 Checking available Microsoft 365 services...');
      const serviceEndpoints = [
        { name: 'Mail', endpoint: 'https://graph.microsoft.com/v1.0/me/mailboxSettings' },
        { name: 'Calendar', endpoint: 'https://graph.microsoft.com/v1.0/me/calendar' },
        { name: 'Contacts', endpoint: 'https://graph.microsoft.com/v1.0/me/contacts?$top=1' },
        { name: 'Tasks', endpoint: 'https://graph.microsoft.com/v1.0/me/todo/lists?$top=1' },
        { name: 'Planner', endpoint: 'https://graph.microsoft.com/v1.0/me/planner/tasks?$top=1' }
      ];
      ...
      for (const service of serviceEndpoints) {
        try {
          ...
          if (response.ok && !data.error) {
            ...
            console.log(`❌ ${service.name} service unavailable: ${data.error?.message}`);
          }
        } catch (error) {
      ...
      console.log('❌ File access blocked - this is common for corporate accounts');
      console.log('✅ Available services:', availableServices.join(', '));
      console.log('💡 BUSINESS STANDARD RECOMMENDATIONS:');
      console.log('   1. Your IT admin may have restricted third-party app file access');
      console.log('   2. OneDrive for Business may require additional permissions');
      console.log('   3. SharePoint access might be limited by organizational policies');
      console.log('   4. Demo mode shows all features working with realistic data');
      console.log('   5. Priority Dashboard will work with available services (Mail, Calendar, Tasks)');
      console.log('   6. Contact IT to enable "Files.Read" and "Sites.Read.All" permissions');
  ...
  const getDocumentType = (fileName: string, mimeType?: string): OfficeDocument['type'] => {
  ...
  const generateDocumentSummary = (fileName: string, type: OfficeDocument['type']): string => {
  ...
  const detectAccountType = async () => {
    try {
      ...
      const isPersonalDomain = email.includes('outlook.com') || 
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      if (debugResult.success && debugResult.fileResult && debugResult.fileResult.data) {
        console.log(`✅ SUCCESS: Found files via ${debugResult.fileResult.source}`);
        
        const rawFiles = debugResult.fileResult.data;
        const officeDocuments: OfficeDocument[] = rawFiles
          .filter((item: any) => {
            const fileName = item.name || item.resourceVisualization?.title || '';
          ...
          .map((item: any): OfficeDocument => {
            const fileName = item.name || item.resourceVisualization?.title || 'Unknown Document';
        ...
        console.log('ℹ️ Using demo documents - Business Standard account with file access restrictions');
        setDocuments(createDemoDocuments());

...
const TRANSPORT_DATA = {
  drivers: [
    {
      ...
      hourlyRate: 18.5,
      overtimeRate: 27.75,
      skills: ["double_decker", "single_decker"],
    ...
    },
    {
      ...
      maxHours: 48,
      currentHours: 35,
      availableShifts: ["night", "early"],
      location: "Pitsea",
      hourlyRate: 20.0,
      overtimeRate: 30.0,
      skills: ["double_decker", "articulated"],
      status: "available" as const,
    },
    {
      id: "D004",
    ...
    },
    {
      ...
      hourlyRate: 21.0,
      overtimeRate: 31.5,
      skills: ["double_decker", "articulated", "minibus"],
    ...
    },
    {
      ...
      hourlyRate: 18.75,
      overtimeRate: 28.13,
      skills: ["single_decker", "double_decker"],
      status: "available" as const,
    },
  ],
  vehicles: [
  ...
  routes: [
    {
      id: "R001",
      name: "Route 12: Southend - London",
      startTime: "06:00",
      endTime: "14:00",
      vehicleRequired: "Double Decker",
      driverRequired: "PCV+",
      passengerLoad: 75,
    ...
    {
      id: "R002",
      name: "Route 45: Wickford Circle",
      startTime: "07:30",
      endTime: "15:30",
      vehicleRequired: "Single Decker",
      driverRequired: "PCV",
      passengerLoad: 35,
    ...
    {
      id: "R003",
      name: "Route 67: Pitsea Express",
      startTime: "14:00",
      endTime: "22:00",
      vehicleRequired: "Articulated",
      driverRequired: "PCV+",
      passengerLoad: 95,
      priority: "high" as const,
      cost: 200,
    ...
    {
      id: "R006",
      name: "Route 34: School Run",
      startTime: "07:00",
      endTime: "09:00",
      vehicleRequired: "Minibus",
      driverRequired: "PCV",
      passengerLoad: 12,
      priority: "high" as const,
      cost: 60,
...
export class SmartRosterOptimizer {
  ...
  async loadRosterFromExcel(
    ...
    return {
      success: true,
      message: `Loaded ${this.shifts.length} shifts from ${fileName}`,
    };
  }
      ...
      if (shift.driverId) {
        ...
        if (
          ...
          conflicts.push({
            ...
            severity: "critical",
            description: `Driver ${
              driver?.name || shift.driverId
            } unavailable for ${this.getRouteById(shift.routeId)?.name}`,
            affectedShifts: [shift.id],
            suggestedResolution:
              "Reassign to available driver with matching skills",
          });
        }
      }
      ...
      if (shift.vehicleId) {
        ...
        if (
          ...
          conflicts.push({
            ...
            severity: "high",
            description: `Vehicle ${shift.vehicleId} unavailable for ${
              this.getRouteById(shift.routeId)?.name
            }`,
            affectedShifts: [shift.id],
  ...
  private async generateOptimalSolution(
    conflicts: Conflict[]
  ): Promise<Change[]> {
    ...
    for (const conflict of conflicts) {
      switch (conflict.type) {
        case "driver_overlap":
          const reassignment = this.findBestDriverReassignment(
            conflict.affectedShifts[0]
          );
          if (reassignment) {
        ...
        case "vehicle_overlap":
          const vehicleReassignment = this.findBestVehicleReassignment(
            conflict.affectedShifts[0]
          );
        ...
        case "overtime_violation":
          const redistribution = this.redistributeHours(
            conflict.affectedShifts
          );
  ...
  private findBestDriverReassignment(shiftId: string): Change | null {
    ...
    const availableDrivers = this.drivers.filter(
      (driver) =>
        ...
        driver.skills.some((skill) =>
          skill.includes(route.vehicleRequired.toLowerCase().replace(" ", "_"))
        ) &&
  ...
  private findBestVehicleReassignment(shiftId: string): Change | null {
    ...
    const availableVehicles = this.vehicles.filter(
      (vehicle) =>
        vehicle.status === "available" && vehicle.type === route.vehicleRequired
    );
  ...
  private calculateMetrics(changes: Change[]) {
    const originalCost = this.routes.reduce(
      (total, route) => total + route.cost,
      0
    );
    const optimizedCost = changes.reduce(
      (total, change) => total + change.cost,
      0
    );
  ...
  private calculateCoverage(): number {
    const assignedShifts = this.shifts.filter(
      (s) => s.driverId && s.vehicleId
    ).length;
  ...
  private generateRecommendations(
    ...
    if (conflicts.some((c) => c.type === "driver_overlap")) {
      recommendations.push(
        "Consider hiring part-time drivers for emergency coverage"
      );
    }

    if (conflicts.some((c) => c.type === "vehicle_overlap")) {
      recommendations.push(
        "Implement preventive maintenance scheduling to avoid conflicts"
      );
    }

    if (changes.length > 3) {
      recommendations.push(
        "Review original roster planning to minimize future disruptions"
      );
    }

    recommendations.push(
      "Set up automated alerts for potential conflicts 24 hours in advance"
    );
    ...
    return [
      {
        ...
        description: `Smart reallocation completed. ${
          optimization.changes.length
        } changes applied, £${optimization.savings.toFixed(0)} saved.`,
        data: {
          trigger,
          optimization,
      ...
      },
      {
        ...
        description: `Operational efficiency improved by ${(
          optimization.efficiency - 80
        ).toFixed(1)}% with smart reallocation`,
        data: {
          cost_analysis: {

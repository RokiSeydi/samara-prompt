...
export class RealExcelProcessor {
  ...
  private extractDriverData(workbook: XLSX.WorkBook): DriverRecord[] {
    ...
    return jsonData.map((row: any, index: number) => ({
      ...
      licenseType:
        row.License || row.license || row["License Type"] || "Standard",
  ...
  private extractVehicleData(workbook: XLSX.WorkBook): VehicleRecord[] {
    ...
    return jsonData.map((row: any, index: number) => ({
      ...
      status: this.normalizeStatus(
        row.Status || row.status
      ) as VehicleRecord["status"],
      fuel_cost_per_km: Number(row["Fuel Cost Per KM"] || row.fuel_cost || 0.8),
      maintenance_cost: Number(
        row["Maintenance Cost"] || row.maintenance_cost || 0.15
      ),
    }));
  }
  ...
  async optimizeRoster(
    ...
    const optimizedDrivers = await this.optimizeWithCompliance(
      drivers,
      routes,
      constraints
    );
    const optimizedVehicles = this.assignVehicles(
      optimizedDrivers,
      vehicles,
      routes
    );
  ...
  async applyCommandChanges(command: string, currentData: any): Promise<any> {
    ...
    const reoptimizedDrivers = await this.optimizeWithCompliance(
      updatedDrivers,
      routes
    );
    const reoptimizedVehicles = this.assignVehicles(
      reoptimizedDrivers,
      updatedVehicles,
      routes
    );
    ...
    return {
      ...
      changes: [
        ...currentData.changes,
        ...this.calculateChanges(drivers, reoptimizedDrivers),
      ],
  ...
  private async optimizeWithCompliance(
    drivers: DriverRecord[],
    routes: RouteRecord[],
    constraints?: any
    ...
    const sortedRoutes = routes.sort((a, b) => {
      ...
      return (
        this.parseTime(a.time_window_start) -
        this.parseTime(b.time_window_start)
      );
    });
    ...
    for (const route of sortedRoutes) {
      ...
      if (availableDriver) {
        ...
        console.log(
          `✅ Assigned ${route.name} to ${
            availableDriver.name
          } (${routeHours.toFixed(
            1
          )}h) - Total: ${availableDriver.currentHours.toFixed(1)}h`
        );
      } else {
        console.warn(
          `⚠️ Could not assign route ${route.name} - no compliant driver available`
        );
      }
    }
    ...
    if (violations.length > 0) {
      console.error(
        `🚨 COMPLIANCE VIOLATION: ${violations.length} drivers exceed ${maxHours}h limit:`,
  ...
  private findBestDriver(
    ...
    const candidates = drivers.filter((driver) => {
      ...
      const driverAssignments = assignmentLog.filter(
        (a) => a.driver === driver.id
      );
      for (const assignment of driverAssignments) {
        ...
        if (
          routeStartTime < assignment.endTime + breakNeeded &&
          routeEndTime > assignment.startTime - breakNeeded
        ) {
  ...
  private hasRequiredLicense(
    ...
    const driverLevel =
      licenseHierarchy[driver.licenseType as keyof typeof licenseHierarchy] ||
      1;
    const requiredLevel =
      licenseHierarchy[
        route.required_license as keyof typeof licenseHierarchy
      ] || 1;
  ...
  private calculateChanges(
    original: DriverRecord[],
    optimized: DriverRecord[]
  ) {
    const changes = [];
    let reassignments = 0;
  ...
  private calculateMetrics(
    ...
    const assignedCount = optimized.filter(
      (d) => d.status === "assigned"
    ).length;
    const totalCount = optimized.length;
    const complianceViolations = optimized.filter(
      (d) => d.currentHours > 8
    ).length;
    ...
    return {
      ...
      conflicts_resolved: Math.max(
        0,
        original.filter((d) => d.currentHours > 8).length - complianceViolations
      ),
      overtime_hours_saved: Math.max(
        0,
        original.reduce((sum, d) => sum + Math.max(0, d.currentHours - 8), 0)
      ),
    };
  }
  ...
  exportToExcel(optimizationResult: any): Blob {
    ...
    const driversWS = XLSX.utils.json_to_sheet(
      optimizationResult.optimizedData.drivers
    );
    const vehiclesWS = XLSX.utils.json_to_sheet(
      optimizationResult.optimizedData.vehicles
    );
    const routesWS = XLSX.utils.json_to_sheet(
      optimizationResult.optimizedData.routes
    );
  ...
  private normalizeStatus(status: string): string {
    ...
    if (
      ["sick", "vacation", "maintenance", "unavailable"].includes(normalized)
    ) {
      return normalized;
    }
    return "available";
    ...
    return [
      {
        ...
        required_license: "Standard",
        passenger_capacity_needed: 30,
        priority: "high",
  ...
  private parseCommand(command: string): {
    ...
    if (
      cmd.includes("sick") &&
      (cmd.includes("mark") || cmd.includes("driver"))
    ) {
      const nameMatch =
        command.match(/mark\s+([^,]+)\s+as\s+sick/i) ||
        command.match(/([^,]+)\s+is\s+sick/i);
    ...
    // Vehicle maintenance
    if (
      cmd.includes("maintenance") &&
      (cmd.includes("bus") || cmd.includes("vehicle"))
    ) {
  ...
  private prioritizeRoutes(routes: RouteRecord[], routeType: string) {
    routes.forEach((route) => {
      if (
        route.name.toLowerCase().includes(routeType.toLowerCase()) ||
  ...
  private assignVehicles(
    ...
    const updatedVehicles = vehicles.map((v) => ({
      ...v,
      status: "available" as VehicleRecord["status"],
    }));
    ...
    drivers.forEach((driver) => {
      if (driver.route && driver.status === "assigned") {
        ...
        if (route) {
          ...
          const suitableVehicle = updatedVehicles.find(
            (v) =>
              v.status === "available" &&
  ...
  private generateRecommendations(
    ...
    const unassignedRoutes = routes.filter(
      (route) => !drivers.some((d) => d.route === route.id)
    );

    if (unassignedRoutes.length > 0) {
      recommendations.push(
        `Consider hiring additional drivers for ${unassignedRoutes.length} unassigned routes`
      );
    }

    const avgHours =
      drivers.reduce((sum, d) => sum + d.currentHours, 0) / drivers.length;
    if (avgHours < 6) {
      recommendations.push(
        "Driver utilization is low - consider route consolidation"
      );
    }

    const complianceRate =
      (drivers.filter((d) => d.currentHours <= 8).length / drivers.length) *
      100;
    if (complianceRate === 100) {
      recommendations.push(
        "All drivers comply with 8-hour daily limits - excellent compliance!"
      );
    }

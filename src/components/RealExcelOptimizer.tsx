...
export const RealExcelOptimizer: React.FC = () => {
  ...
  const checkCompliance = (optimizationResult: any) => {
    ...
    optimizationResult.optimizedData.drivers.forEach((driver: any) => {
      if (driver.currentHours > 8.0) {
        issues.push({
          type: "hours_exceeded",
          severity: "high",
          description: `${driver.name} assigned ${driver.currentHours.toFixed(
            1
          )} hours (exceeds 8-hour limit)`,
        ...
        issues.push({
          ...
          description: `${driver.name} assigned ${driver.currentHours.toFixed(
            1
          )} hours (approaching 8-hour limit)`,
    ...
    optimizationResult.optimizedData.routes.forEach((route: any) => {
      const assignedDriver = optimizationResult.optimizedData.drivers.find(
        (d: any) => d.route === route.id
      );
      if (
        assignedDriver &&
        route.required_license &&
        assignedDriver.licenseType !== route.required_license
      ) {
        issues.push({
    ...
    optimizationResult.optimizedData.routes.forEach((route: any) => {
      ...
      if (assignedDriver && assignedDriver.vehicle) {
        const assignedVehicle = optimizationResult.optimizedData.vehicles.find(
          (v: any) => v.id === assignedDriver.vehicle
        );
        if (
          assignedVehicle &&
          route.passenger_capacity_needed > assignedVehicle.capacity
        ) {
          issues.push({
    ...
    const unassignedRoutes = optimizationResult.optimizedData.routes.filter(
      (route: any) =>
        !optimizationResult.optimizedData.drivers.some(
          (d: any) => d.route === route.id
        )
    );
    ...
    if (unassignedRoutes.length > 0) {
      issues.push({
        ...
        suggestion:
          "Consider hiring additional drivers or adjusting route schedules",
      });
    }

    // Calculate compliance score
    const totalDrivers = optimizationResult.optimizedData.drivers.length;
    const compliantDrivers = optimizationResult.optimizedData.drivers.filter(
      (d: any) => d.currentHours <= 8.0
    ).length;
    ...
    const driverComplianceRate =
      totalDrivers > 0 ? (compliantDrivers / totalDrivers) * 100 : 100;
    const routeCompletionRate =
      totalRoutes > 0 ? (assignedRoutes / totalRoutes) * 100 : 100;
    const overallScore = Math.round(
      driverComplianceRate * 0.7 + routeCompletionRate * 0.3
    );
  ...
  const handleDownloadOptimized = () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `optimized_roster_${
  ...
  const handleCommandSubmit = async () => {
    ...
    try {
      ...
      const updatedResult = await realExcelProcessor.applyCommandChanges(
        command,
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center", marginBottom: "40px" }}>
        <SparkleRegular
          style={{ fontSize: "48px", color: "#0078D4", marginBottom: "16px" }}
        />
        <Text
          style={{
            fontSize: "32px",
        ...
        >
          Upload your transportation roster Excel file to optimize driver
          assignments, vehicle allocation, and route scheduling with AI-powered
          insights.
        </Text>
      </div>
      ...
      {!fileState.uploaded && (
        ...
        >
          ...
          <Text style={{ marginBottom: "16px", display: "block" }}>
            Need a sample file to try out the optimizer? Download our sample
            transportation roster with realistic data:
          </Text>
          <Button
            appearance="outline"
          ...
          <Text style={{ fontSize: "14px", color: "#666", display: "block" }}>
            The sample file contains 25 drivers, 20 vehicles, and 15 routes with
            realistic transportation data.
          </Text>
        </Card>
      )}
      ...
      {optimizationState.processing && (
        <Card style={{ padding: "30px", textAlign: "center" }}>
          ...
          >
            Analyzing driver assignments, vehicle allocation, and route
            optimization...
          </Text>

          <ProgressBar
            value={optimizationState.progress}
            max={100}
        ...
        <div>
          ...
          <Card
            style={{
              padding: "24px",
              marginBottom: "20px",
              backgroundColor: optimizationState.result.compliance?.isCompliant
                ? "#f3f9fd"
                : "#fff4f4",
            }}
          >
              ...
              <div>
                <Text
                  style={{
                    fontSize: "24px",
                    fontWeight: 600,
                    color: optimizationState.result.compliance?.isCompliant
                      ? "#107C10"
                      : "#D13438",
                    display: "block",
                  }}
            ...
            >
              ...
              >
                ...
                <Text style={{ fontSize: "16px", fontWeight: 600 }}>
                  {
                    optimizationState.result.optimizedData.drivers.filter(
                      (d: any) => d.currentHours <= 8
                    ).length
                  }
                  ...
                  >
                    (
                    {optimizationState.result.compliance
              ...
              >
                ...
                <Text style={{ fontSize: "16px", fontWeight: 600 }}>
                  {
                    optimizationState.result.optimizedData.routes.filter(
                      (r: any) =>
                        optimizationState.result.optimizedData.drivers.some(
                          (d: any) => d.route === r.id
                        )
                    ).length
                  }
                  ...
                  >
                    (
                    {optimizationState.result.compliance?.routeCompletionRate ||
                      0}
                    %)
                  </Text>
                </Text>
              ...
              >
                ...
                <Text
                  style={{
                    ...
                    fontWeight: 600,
                    color:
                      (optimizationState.result.compliance?.score || 0) >= 90
                        ? "#107C10"
                        : (optimizationState.result.compliance?.score || 0) >=
                          70
                        ? "#F7630C"
                        : "#D13438",
                  }}
          ...
          <Card style={{ padding: "24px", marginBottom: "20px" }}>
            ...
            >
              Tell me what changes you need and I'll update the optimization
              accordingly:
            </Text>
          ...
          >
            ...
            >
              ...
              <div style={{ textAlign: "center" }}>
                ...
                >
                  {optimizationState.result.metrics.efficiency_improvement.toFixed(
                    1
                  )}
                  %
                </Text>
